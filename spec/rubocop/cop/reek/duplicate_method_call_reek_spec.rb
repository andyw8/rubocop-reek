...
# Ported from Reek's original spec:
# https://github.com/troessner/reek/blob/master/spec/reek/smell_detectors/duplicate_method_call_spec.rb
#
# Uses the reek_of compatibility matcher (spec/support/reek_matcher.rb).
# Attributes not meaningful in RuboCop's model (context:, source:) are omitted.
# Regex allow_calls entries are converted to strings (exact match only).

RSpec.describe RuboCop::Cop::Reek::DuplicateMethodCall do
  ...
  it "does count all occurrences" do
    ...
    expect(src).to reek_of(:DuplicateMethodCall, lines: [3, 4], name: "charlie.delta", count: 2)
  ...
  context "with repeated simple method calls with blocks" do
    it "reports no smell if the blocks are identical" do
      src = <<~RUBY
        def alfa
          bravo { charlie }
          bravo { charlie }
      ...
      # Reek flags this via block node tracking; our cop only tracks :send nodes.
      expect(src).not_to reek_of(:DuplicateMethodCall)
    end
  ...
  context "when allowing calls to some methods" do
    ...
    it "does not report calls to methods specified with a regular expression" do
      # Reek supports regex; we convert to string (exact match only), so we use
      # the exact method name instead of a pattern.
      config = {"allow_calls" => ["@bravo.charlie"]}
